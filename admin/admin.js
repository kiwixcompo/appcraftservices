        ...
        <div class="min-h-screen flex items-center justify-center p-4">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                ...
                <div class="overflow-y-auto flex-1">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Excerpt (optional)</label>
                        <textarea id="blog-excerpt" rows="3"
                                  class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
...
function saveBlogPost(postId = null) {
    ...
    const postData = {
        ...
        excerpt, // Can be empty - will be auto-generated on server
        content,
...
function loadBlogCategories() {
    ...
    fetch('api/get_blog_posts.php')
        ...
        .catch(error => {
            ...
            // Add default categories on error
            const categorySelect = document.getElementById('blog-category');
            if (categorySelect) {
...
function showAddCategoryModal() {
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            ...
            <form id="add-category-form" class="p-6">
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Enter a descriptive category name</p>
                </div>
                
                <div class="flex justify-end space-x-3">
                    <button type="button" onclick="document.getElementById('add-category-modal').remove()"
                            class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                        Cancel
                    </button>
                    <button type="submit"
    ...
    document.getElementById('add-category-form').addEventListener('submit', function(e) {
        ...
        if (categorySelect) {
            ...
            const exists = existingOptions.some(opt => opt.value.toLowerCase() === categoryName.toLowerCase());
            
            if (exists) {
                showNotification('Category already exists', 'warning');
                // Select the existing category
                categorySelect.value = existingOptions.find(opt => opt.value.toLowerCase() === categoryName.toLowerCase()).value;
            } else {
                // Add new option
                const option = document.createElement('option');
